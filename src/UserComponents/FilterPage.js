...
const FilterPage = () => {
    ...
    useEffect(() => {
        if (selectedFilter === 'filter1' && selectedSchool) {
            ...
            axios.get(`${API_BASE_URL}/filter/school/item_type?schoolCode=${selectedSchool}`)
                .then(response => setFilteredItemTypeList(response.data))
                .catch(error => console.error('Error fetching item types for school:', error));
        }
    ...
    useEffect(() => {
        if (selectedFilter === 'filter1' && selectedSchool && selectedItemType) {
            ...
            axios.get(`${API_BASE_URL}/filter/item_category/item_type`, {
                params: {
                ...
                .catch(error => console.error('Error fetching data for Filter 1:', error));
        } else if (selectedFilter === 'filter2' && selectedItemType) {
            // Fetch data based on the selected item type for Filter 2
            axios.get(`${API_BASE_URL}/filter/item_list_type`, {
                params: {
                    itemType: selectedItemType,
                },
            })
                .then(response => setData(response.data))
                .catch(error => console.error('Error fetching data for Filter 2:', error));
        } else if (selectedFilter === 'filter3' && selectedSchool) {
            // Fetch data based on the selected school for Filter 3
            axios.get(`${API_BASE_URL}/filter/item_list_school_code?schoolCode=${selectedSchool}`)
                .then(response => setData(response.data))
                .catch(error => console.error('Error fetching data for Filter 3:', error));
        }
